...
class Generator(nn.Module):
    def __init__(
        ...
        # BatchNorm
        self.batchnorm1D = nn.BatchNorm1d(input_dim * output_dim)
...
class Discriminator(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int):
        ...
        self.input_conv = nn.Conv2d(
            1, out_channels=4, kernel_size=(2, 7), padding="valid"
        )
        nn.init.xavier_uniform_(self.input_conv.weight)
        self.hidden_conv = nn.Conv2d(
            4, out_channels=4, kernel_size=(2, 7), padding="valid"
        )
        nn.init.xavier_uniform_(self.hidden_conv.weight)
        self.output_conv = nn.Conv2d(
            4, out_channels=1, kernel_size=(1, 7), padding="valid"
        )
